...
export default function SignInPrompt({
  ...
  email: string;
  /** Caller's chance to stash state (sessionStorage etc.) before the OAuth jump. */
  onBeforeRedirect?: () => void;
  /** Path the OAuth callback will land on. */
  redirectNext?: string;
}) {
  ...
  async function handleSignIn() {
    ...
    try {
      ...
      const { error: authError } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          redirectTo,
          // Pre-fill the Google account picker when possible.
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-start gap-2 rounded-md border border-ink-12 bg-cream px-3 py-2.5 text-xs text-navy-mid">
        <ShieldCheck className="mt-0.5 h-4 w-4 flex-none text-amber" />
        <span>
          Sign in to download. We use Google sign-in so your purchase stays
          attached to your account — no passwords.
        </span>
      </div>
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-md border border-ink-22 bg-white px-4 py-2.5 text-sm font-semibold text-navy transition hover:bg-cream disabled:cursor-not-allowed disabled:opacity-60"
      >
        {state === "redirecting" ? (
          <>
